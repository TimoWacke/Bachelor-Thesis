...
It holds the converter itself, to minimize lines of code in the main script.
The class is mainly used to manage the access to the different files and data, before and after the conversion.
And secondly to detect the gaps in the data, for infilling simply in the form of listing the hours where data is missing.
And for training use cases additionally in form of listing the months where at least some data is available which is most convenient for the API applicance to get ERA5 data for the station.
...
Because CRAI is using the encode, decode architecture with connected layers as described in \autoref{subsec:cnn}, the output file in \code{NetCDF} format that the model produces includes not only one temperature value per timestep but has the same dimensions as the input ERA5 file meaning it uses the same 8x8 grid with 64 temperature values. However since it has been trained on input files where the ground truth was also laid on the 8x8 grid, the 64 temperature values in the output data of the model tend to be extremely similar.
As a standard to compare the the groud truth with the reconstruction and the ERA5 data as in \autoref{sec: results}, a method is implemented that takes in a file for each data. The output file of the model and the ERA5 file for the model have the same spatial dimensions spanning over the 64 coordinates. The file for the measurements is the one created by \code{DatToNcConverter} which is different to the 

To efficiently make this transformation and establish a standard again a method is implemented, that takes in the output file of the model and the training dataset file with the measurements and creates a  
